...
from myspa.models import MassageTherapist, Position, Procedure, Review, Salon, Schedule, SpaUser, TypeCategories, \
    SpaСategories
...
class RegisterUserForm(UserCreationForm):
    first_name = forms.CharField(label="Iм'я",
                                 widget=forms.TextInput(attrs={'class': 'form-input-name', 'placeholder': "Iм'я"}))
    last_name = forms.CharField(label="Прізвище",
                                widget=forms.TextInput(attrs={'class': 'form-input-name', 'placeholder': "Прізвище"}))
    phone = forms.CharField(label='Телефон', widget=forms.TextInput(
        attrs={'class': 'form-input', 'placeholder': 'Телефон', 'type': 'tel'}))
    email = forms.CharField(label='E-mail',
                            widget=forms.EmailInput(attrs={'class': 'form-input', 'placeholder': 'E-mail'}))
    ...
    password2 = forms.CharField(label="Повторіть пароль", widget=forms.PasswordInput(
        attrs={'class': 'form-input', 'placeholder': "Повторіть пароль"}))
...
class LoginUserForm(AuthenticationForm):
    email = forms.EmailField(label='Ваш e-mail', widget=forms.EmailInput(
        attrs={'class': 'form-input', 'placeholder': 'Ваш e-mail', 'id': 'id_email_2'}))
...
class MassageTherapistForm(forms.ModelForm):
    salon = forms.ModelChoiceField(queryset=Salon.objects.all(), label='',
                                   widget=forms.Select(attrs={'class': 'form-input'}))
    position = forms.ModelMultipleChoiceField(queryset=Position.objects.all(), label='', widget=forms.SelectMultiple(
        attrs={'class': 'form-input select', 'size': '10'}))
...
class CategoriesUpdateForm(forms.ModelForm):
    ...
    categories_image = forms.ImageField(label='', widget=forms.FileInput(attrs={'class': 'create_categories_image'}),
                                        required=False)
    description = forms.CharField(label='', widget=forms.Textarea(
        attrs={'class': 'create_description', 'rows': 8, 'placeholder': 'Description'}))
...
class CategoriesAddForm(forms.ModelForm):
    name = forms.CharField(label='',
                           widget=forms.TextInput(attrs={'class': 'add_create_name', 'placeholder': 'Назва категорії'}),
                           required=True)
    categories_image = forms.ImageField(label='',
                                        widget=forms.FileInput(attrs={'class': 'add_create_categories_image'}),
                                        required=True)
    description = forms.CharField(label='', widget=forms.Textarea(
        attrs={'class': 'add_create_description', 'rows': 8, 'placeholder': 'Опис категорії'}), required=True)
...
class MassageTherapistUpdateForm(forms.ModelForm):
    ...
    position = forms.ModelMultipleChoiceField(queryset=Position.objects.all(), widget=forms.SelectMultiple(
        attrs={'class': 'form-input select', 'size': '4'}))
    ...
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(
                attrs={'class': 'form-control-description', 'placeholder': 'Введите описание'}),
